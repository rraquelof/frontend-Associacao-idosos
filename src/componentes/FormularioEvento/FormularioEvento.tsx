...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (metodo === "POST") {
        ...
        resposta = await fetch(`${API_URL}/cadastroEvento`, {
          method: "POST",
          headers: {
        ...
        resposta = await fetch(`${API_URL}/eventos/${eventoId}`, {
          method: "PUT",
          headers: {
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <div className="flex gap-6">
              <div className="flex flex-col w-1/2">
                <Label htmlFor="latitude" texto="Latitude (Localização)" />
                <ErroCampoObrigatorio valor={formDados.latitude} obrigatorio envioVazio={enviarVazio}>
                  <Input
                    type="number"
                    step="any"
                    id="latitude"
                    name="latitude"
                    value={formDados.latitude}
                    onChange={handleChange}
                    required
                  />
                </ErroCampoObrigatorio>
              ...
              <div className="flex flex-col w-1/2">
                <Label htmlFor="longitude" texto="Longitude (Localização)" />
                <ErroCampoObrigatorio valor={formDados.longitude} obrigatorio envioVazio={enviarVazio}>
                  <Input
                    type="number"
                    step="any"
                    id="longitude"
                    name="longitude"
                    value={formDados.longitude}
                    onChange={handleChange}
                    required
                  />
                </ErroCampoObrigatorio>
            ...
            {metodo === "POST" && (
              <div className="flex flex-col">
                <Label htmlFor="imagem" texto="Imagem do Evento (Opcional)" />
                <input
                  type="file"
                  id="imagem"
                  name="imagem"
